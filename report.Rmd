...
caption_4 <- 'Variance explained by each principal component (PC) from PCA of 
the example intensity data. Blue bars represent the proportion of variance explained 
by each individual PC, while the red line and points display the cumulative proportion 
of variance explained as additional PCs are included. PCs are ordered left to right 
from most to least variance explained (PC1, PC2, PC3, etc.). The cumulative variance 
curve approaching 1.0 indicates the total number of PCs required to capture the 
majority of variability in the dataset.'
caption_5 <- 'Principal component analysis (PCA) biplot of PC1 versus PC2 
for the example intensity data. Each point represents an individual sample, colored 
by its SixSubtypesClassification label. The x-axis (PC1) and y-axis (PC2) display 
the proportion of total variance explained by each principal component, respectively. 
Clustering of points by color indicates that gene expression profiles separate 
by subtype, while overlap between colors suggests shared expression patterns 
across subtypes.'
caption_8 <- 'Heatmap of intensity values for differentially expressed probes 
identified at an FDR threshold of 0.01. Each row represents an individual probe and 
each column represents a sample. Intensity values are row-normalized (z-scored) to 
highlight relative expression differences across samples. Rows and columns are 
hierarchically clustered to reveal co-expression patterns among probes and groupings 
among samples. Color intensity reflects the normalized expression level, with one end 
of the RdBu palette representing high expression and the other representing low 
expression.'
```
...
variance_tibble$principal_components <- factor(
    variance_tibble$principal_components, 
    levels = variance_tibble$principal_components
)
